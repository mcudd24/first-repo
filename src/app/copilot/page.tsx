...
import { Send, Sparkles, MailCheck, BellPlus } from "lucide-react";
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-8rem)] max-w-3xl flex-col md:h-[calc(100dvh-5rem)]">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pb-4">
        {messages.length === 0 && (
          <div className="stagger grid gap-2 pt-6 sm:grid-cols-2">
            {SUGGESTIONS.map((s) => (
              <button
                key={s}
                onClick={() => send(s)}
                className="glass p-4 text-left text-sm text-slate-600 transition-all hover:scale-[1.02] hover:text-accent-500 dark:text-slate-300"
              >
                “{s}”
              </button>
            ))}
      ...
      <form
        ...
        className="glass flex items-center gap-2 p-2 animate-fade-up"
      >
        <Input
          value={input}
